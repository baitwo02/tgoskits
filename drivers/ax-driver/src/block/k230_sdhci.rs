...
fn probe(probe: ProbeFdt<'_>) -> Result<(), OnProbeError> {
    ...
    let irq_source = probe.register_block(dev);
...
struct BlockDevice {
    raw: Option<SharedDriver<K230Sdhci>>,
    capacity_blocks: u64,
        ...
        if let Some(raw) = &self.raw {
            ...
            raw.with_mut(|raw| {
                if let Err(err) = SdioHost::enable_completion_irq(raw.host_mut()) {
                    warn!("k230-sdhci: enable completion IRQ failed: {:?}", err);
                    return;
                }
                enabled = true;
            });
        ...
        if let Some(raw) = &self.raw {
            raw.with_mut(|raw| {
                if let Err(err) = SdioHost::disable_completion_irq(raw.host_mut()) {
                    warn!("k230-sdhci: disable completion IRQ failed: {:?}", err);
                }
            });
        }
...
impl BlockQueue {
    fn new(raw: SharedDriver<K230Sdhci>, capacity_blocks: u64, id: usize) -> Self {
        Self {
            raw,
            capacity_blocks,
            id,
    ...
    fn poll_active_request(
        ...
        match raw.with_mut(|raw| {
            ...
            Ok(_) => Err(rdif_block::BlkError::Other(
                "K230 SDHCI returned an unknown poll state",
            )),
