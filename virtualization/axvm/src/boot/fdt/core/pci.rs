...
use super::tree::{FdtTree, prop_string, prop_u32_list};
...
fn add_host_node(tree: &mut FdtTree, host: GuestPciHost) -> AxVmResult {
    ...
    tree.set_property(node_id, prop_u32_list("#address-cells", &[3]))?;
    tree.set_property(node_id, prop_u32_list("#size-cells", &[2]))?;
    tree.set_property(node_id, prop_u32_list("#interrupt-cells", &[1]))?;
    tree.set_property(node_id, prop_u32_list("linux,pci-domain", &[0]))?;
    tree.set_property(node_id, prop_u32_list("bus-range", &[0, 0]))?;
    tree.set_property(
        node_id,
        cell_property(
            "reg",
