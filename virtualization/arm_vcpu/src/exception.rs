...
pub fn handle_exception_sync(ctx: &mut TrapFrame) -> ArmVcpuResult<ArmVmExit> {
    match exception_class() {
        ...
        Some(ESR_EL2::EC::Value::HVC64) => {
            ...
            // ELR_EL2 already holds the post-HVC return address. Advancing it
            // again would skip the guest instruction that consumes x0.
...
mod tests {
    ...
    #[test]
    fn hvc_psci_version_preserves_hardware_return_pc() {
        ...
        let exit = handle_hvc_psci_version(&mut ctx)
            .expect("PSCI_VERSION should use the fast path")
            .expect("PSCI_VERSION HVC should succeed");
    ...
    #[test]
    fn generic_hvc_preserves_hardware_return_pc() {
        let mut ctx = TrapFrame::default();
        ctx.set_exception_pc(TEST_PC);
