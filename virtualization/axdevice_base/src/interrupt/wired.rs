...
pub enum IrqError {
    ...
    InvalidTriggerMode {
        ...
        expected: InterruptTrigger,
        /// Trigger mode configured for the input.
        actual: InterruptTrigger,
    },
...
///
/// [`WiredIrqInput`] performs per-source wired-OR accounting before invoking
/// this capability.
...
impl WiredIrqInput {
    ...
    /// Creates a separately identified device connection to this input.
...
impl IrqLine {
    ...
    pub fn assert(&self) -> IrqResult {
        self.set_asserted("assert", true)
    }

    fn set_asserted(&self, operation: &'static str, asserted: bool) -> IrqResult {
        self.require_trigger(operation, InterruptTrigger::LevelTriggered)?;
        let _transition = SerialTransition::acquire(&self.0.level_transition);
        if self.0.asserted.load(Ordering::Relaxed) == asserted {
            return Ok(());
        }
        if asserted {
        ...
        self.0.asserted.store(asserted, Ordering::Relaxed);
        Ok(())
    }

    /// Deasserts this level-triggered source.
    pub fn deassert(&self) -> IrqResult {
        self.set_asserted("deassert", false)
    }

    /// Delivers one edge from this source.
    pub fn pulse(&self) -> IrqResult {
        self.require_trigger("pulse", InterruptTrigger::EdgeTriggered)?;
    ...
    fn require_trigger(&self, operation: &'static str, expected: InterruptTrigger) -> IrqResult {
