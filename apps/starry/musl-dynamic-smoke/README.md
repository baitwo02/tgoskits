...
```bash
cargo xtask starry app qemu -t musl-dynamic-smoke --arch aarch64
cargo xtask starry app qemu -t musl-dynamic-smoke --arch riscv64
cargo xtask starry app qemu -t musl-dynamic-smoke --arch x86_64
```
...
| lld | clang 链接器，`apt-get install lld-14`（提供 `ld.lld`、`lld` 或 Rust 工具链内置 `rust-lld` 任一即可，`prebuild.sh` 自动识别） |
...
8. **声明依赖满足时可直接复现**：按"环境依赖"表装齐 `clang`、`ld.lld`/`lld`/`rust-lld`、`debugfs`、`qemu-system-*` 与 musl rootfs 后，"测试命令"一节中的 3 条 `cargo xtask starry app qemu -t musl-dynamic-smoke --arch {aarch64,riscv64,x86_64}` 可直接复现三架构 PASS，无需额外 PATH shim。
