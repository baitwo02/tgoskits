...
AArch64 使用同一个 typed provider 和 root binding：host 节点从架构 auto-MMIO pool 取得 1 MiB ECAM 与完整 64 MiB memory aperture，ECAM frontend 和 aperture frontend 只委托同一 `PciRootState`。只有 topology 含 endpoint 时，架构固件适配器才从 graph-resolved ranges 生成 `pci-host-ecam-generic` 节点；没有 endpoint 时不改变 guest DTB。已有 PCI bridge、冲突 top-level `reg`、无 DTB endpoint 或不满足 1 MiB/sub-4 GiB 契约时明确失败，不覆盖或猜测 fallback。
